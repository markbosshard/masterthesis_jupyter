 ...
 */
   ...
   function initMe(pageType) {
        ...
        myDiagram.nodeTemplateMap.add("Milestone",  // the default category
            $$(go.Node, "Spot", nodeStyle(), {resizable: true, name: "resizeR", resizeObjectName: "R", movable: true, maxLocation: new go.Point(Infinity, NaN), minLocation: new go.Point(-Infinity, NaN)},
                // the main object is a Panel that surrounds a TextBlock with a rectangular Shape
                $$(go.Panel, "Auto",
                    $$(go.Shape, "Rectangle",
                    ...
                    $$(go.TextBlock,
                        ...
                        new go.Binding("text").makeTwoWay()),
                        new go.Binding("resizeObjectName", "resizeObjectName").makeTwoWay()
                )
            ));
...
// function needed for milestones resizing
    function shapeClicked(e, obj) {
      ...
      console.log(obj.desiredSize);
      //var data = myDiagram.model.findNodeDataForKey(node.value.data.key);
     // myDiagram.model.setDataProperty(data, "Osize", obj.desiredSize);
    }
